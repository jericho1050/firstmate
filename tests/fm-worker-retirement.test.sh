...
run_hook() { # <event> [<id>]
  ...
  if [ "$event" = recover ]; then
    PATH="$CASE/fakebin:$PATH" \
      ...
      FM_ROOT_OVERRIDE="$ROOT" FM_TMUX_LOG="$CASE/tmux.log" \
      FM_TREEHOUSE_LOG="$CASE/treehouse.log" FM_FAKE_STATE_FILE="$CASE/crew-state" \
...
test_backend_close_boundary_is_delegated() {
  ...
  assert_no_grep '--force' "$CASE/tmux.log" "retirement hook introduced a force cleanup argument"
