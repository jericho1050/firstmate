...
test_redzone_fence_is_in_every_scaffold() {
  ...
  before=$(mktemp "$TMP_ROOT/brief-before.XXXXXX")
  cp "$home/data/redzone-fence-ship/brief.md" "$before"
  FM_HOME="$home" "$ROOT/bin/fm-brief.sh" redzone-fence-ship firstmate --mode no-mistakes >/dev/null 2>&1
  status=$?
  expect_code 1 "$status" "re-scaffolding an existing brief must fail"
  after=$(mktemp "$TMP_ROOT/brief-after.XXXXXX")
  cp "$home/data/redzone-fence-ship/brief.md" "$after"
  cmp -s "$before" "$after" \
    || fail "re-scaffolding an existing brief rewrote its generated bytes"
