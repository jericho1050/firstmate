...
test_faster_paths_use_configured_authority_without_stacked_review() {
  ...
  brief="$home/data/$id/brief.md"
  assert_grep "The configured merge authority approves the ready branch, then firstmate merges it into the local default branch through the guarded fast-forward path." "$brief" \
    "local-only brief lost configured merge authority and guarded landing"
  assert_no_grep "local \`main\`" "$brief" \
    "local-only brief hard-coded the default branch name"
  assert_no_grep "if \`main\` has advanced" "$brief" \
    "local-only brief hard-coded the default branch update check"
